...
service must use the same value. When it is omitted, the TypeScript server uses
the shared `JWT_SECRET` to sign a short-lived internal service JWT. Supported
periods are `weekly`, `monthly`, `yearly`, `financial_year`, `all_time`, and
`custom`; custom requests require inclusive `start_date` and `end_date` values
in `YYYY-MM-DD` format. Supported roles are `beautician` and `rider`.
